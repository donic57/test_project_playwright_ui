...
class CustomerAccount(BasePage):
    ...
    def fill_login_form(self, first_name, last_name, password, confirm_password, email=None):
        ...
        field_first_name.fill(first_name)
        field_last_name.fill(last_name)
        if email is not None:
            email_address.fill(email)
        field_password.fill(password)
        field_confirm_password.fill(confirm_password)
        button.click()
